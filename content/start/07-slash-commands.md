...
A slash command is a markdown file in `.claude/commands/` inside your project. The filename becomes the command. Save a file called `morning.md` and restart Claude in the folder — type `/` and there it is.

<CourseDiagram id="m07-d1" />

The file has two parts: a short frontmatter block that describes the command, and a body that runs as the prompt, word for word.
...
The command name is the name you'll type under time pressure — treat it like a keyboard shortcut, not a filename.
